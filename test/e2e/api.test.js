...
describe('API E2E test suite', () => {
  ...
  describe('/login', () => {
    ...
    it('should login succesfully given user and password', async () => {
      ...
      const response = await result.json()
      assert.ok(response.token.length, `response.token should be a valid jwt token, actual ${response.token}`)
  ...
  describe('/', () => {
    ...
    it('should not be allowed to access private data with a valid token', async () => {
      const input = {
        headers: {
